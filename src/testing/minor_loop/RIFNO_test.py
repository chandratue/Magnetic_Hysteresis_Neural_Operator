...
# Model
model_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../results/trained_models/RIFNO/minor_loop/'))
model_save_path = os.path.join(model_dir, 'trained_model.pth')

# saving pred and errors
preds_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../results/preds/RIFNO/minor_loop/'))
...
ax.set_title('Preds. RIFNO', fontsize=25)
...
plt.savefig('Minor_RIFNO_preds.png', bbox_inches='tight', dpi=300)
...
# Set title and labels with specific font sizes
ax.set_title('RIFNO', fontsize=25)
...
# Save the figure with a 1:1 aspect ratio
plt.savefig('Minor_RIFNO_ref.png', bbox_inches='tight', dpi=300)
...
# Set title and labels with specific font sizes
ax.set_title('Error RIFNO', fontsize=25)
ax.set_xlabel('$t$ [s]', fontsize=25)
ax.set_ylabel('$B$ [T]', fontsize=25)
...
# Save the figure with a 1:1 aspect ratio
plt.savefig('Minor_RIFNO_error.png', bbox_inches='tight', dpi=300)
...
ax1.plot(H_test[10, :], B_test_pred.detach().numpy()[10, :], '-k', lw=2.0, label="Predicted")

for i in range(10, 20):
    ax1.plot(H_test[i, :], B_test_pred.detach().numpy()[i, :], '-k', lw=2.0)

ax1.plot(H_test[10, :], B_test[10, :], '--r', lw=2.0, label="Actual")
