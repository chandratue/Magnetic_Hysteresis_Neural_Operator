...
# Loss
loss_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../results/logs/EDLSTM/FORC/'))
os.makedirs(loss_dir, exist_ok=True)
train_loss_path = os.path.join(loss_dir, 'train_loss_list.pkl')
test_loss_path = os.path.join(loss_dir, 'test_loss_list.pkl')

# Model
model_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../results/trained_models/EDLSTM/FORC/'))
...
optimizer = torch.optim.Adam(edlstm.parameters(), lr=1e-4)

# Initialize a list to store loss values
loss_history_train = []

# Training loop for Encoder-Decoder LSTM
for epoch in range(num_epochs):
    ...
    encoder_input = input_tensor  # This would be your input sequence
    decoder_input = target_tensor[:, :-1, :]  # Use the target sequence (excluding the last time step) for training
    ...
    # Forward pass through the decoder (decoder_input is the target sequence during training)
    decoder_output, decoder_hidden = edlstm.decoder_lstm(decoder_input, decoder_hidden)

    # Final linear layer to map decoder outputs to the target space
    output = edlstm.fc(decoder_output)

    # Calculate loss using the decoder's output and the actual target sequence (excluding the first time step)
    loss = criterion(output, target_tensor[:, 1:, :])
...
torch.save({'model_state_dict': edlstm.state_dict()}, model_save_path)
